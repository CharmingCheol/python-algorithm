...
2. n까지 수열의 합을 구할 때
   > 1.func(n) = (1+2+...+(1/2)) + ((n/2+1)+(n/2+2)+...+(n/2+n/2))
   > 2.func(n/2)을 하면 앞에 있는 수식인 (1+2+...+(1/2)) 부분만 해당되기에 뒤에 있는 수식을 구해줘야 함
   > 3.((n/2+1)+(n/2+2)+...+(n/2+n/2)) = n/2 * n/2 + (1+2+...+n/2)과 같음
   > 4.(1+2+...+n/2)은 func(n/2)과 같으니, func(n)을 점화식으로 표현한다면,
       func(n) = (n/2) * (n/2) + 2 * func(n/2)가 됨
